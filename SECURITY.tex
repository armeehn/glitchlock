...
\item \textbf{Intra frames are in the clear on H.264 and HEVC.} Those carriers lock motion vectors only, so every intra picture — one per GOP, once a second with the default \ripmono{prepare} settings — decodes exactly as in the carrier. Anyone can pull a clean still from each GOP.

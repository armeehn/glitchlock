...
for i in $(seq 1 10); do
  ...
  ffgac -v error -y -f lavfi -i "testsrc2=s=${W}x${H}:r=25:d=$DUR" -f lavfi -i "nullsrc=s=${W}x${H}:r=25:d=$DUR,geq=random($S)*255:128:128" -filter_complex "[0][1]blend=all_mode=average" -pix_fmt yuv420p -c:v rawvideo $src 2>/dev/null
